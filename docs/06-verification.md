...
默认配置（`$LocalAdminName` 留空）下，镜像不预建任何账户。安装过程与官方介质
一致：依次经过区域、键盘、网络、隐私设置与用户名创建页面。唯一差异是镜像中
的 `BypassNRO=1` 解除了强制联网登录微软账户的要求——在网络页面可选择
"我没有 Internet 连接"，随后创建本地账户。

若在 `config.ps1` 中设置了 `$LocalAdminName`（无人值守模式），安装将自动创建
该本地管理员并跳过 OOBE 页面；此时若 `$LocalAdminPassword` 同时为空，则首次
登录无需输入密码，进入系统后应立即执行 `net user <账户名> *` 设置密码。
